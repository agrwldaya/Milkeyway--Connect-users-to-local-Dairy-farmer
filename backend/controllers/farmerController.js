...
export const completeFarmerProfile = async (req, res) => {
  try {
    ...
    // Validate input
    if (!user_id || !farm_name || !address || !delivery_radius_km) {
      return res.status(400).json({ message: "Farm name, address, and delivery radius are required" });
    }
    ...
    if (existingProfileCheck.rows.length > 0) {
       ...
       await pool.query(
        `UPDATE farmer_profiles 
         ...
         WHERE user_id=$7`,
        [farm_name, address, delivery_radius_km, latitude || null, longitude || null, "draft", user_id]
       );
    }
    else {
    // Insert new profile
    await pool.query(
      ...
      [user_id, farm_name, address, delivery_radius_km, latitude || null, longitude || null, "draft"]
    );
}
...
export const uploadDocuments = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Farmer has been added for admin approval",
      uploaded: {
        farm_image_url,
        farmer_image_url,
...
export const submitProofDocuments = async (req, res) => {
  try {
    const { user_id } = req.params;
    ...
    if (uploadedUrls.length > 0) {
      ...
      return res.status(400).json({ success: false, message: "Failed to upload farmer proof document" });
    }
    ...
    if (existingDocs.rows.length > 0) {
      await pool.query(
        `UPDATE farmer_docs 
         SET farm_image_url=COALESCE($1, farm_image_url), 
             farmer_image_url=COALESCE($2, farmer_image_url), 
             farmer_proof_doc_url=COALESCE($3, farmer_proof_doc_url), 
             updated_at=NOW()
         WHERE farmer_id=$4`,
        [farm_image_url, farmer_image_url, farmer_proof_doc_url, farmer_id]
      );
    ...
    return res.status(200).json({
      success: true,
      message: "Documents submitted successfully. Your account is now pending admin approval.",
      uploaded: {
        farm_image_url,
        farmer_image_url,
        farmer_proof_doc_url
    ...
    console.error("Submit Proof Documents Error:", error);
    res.status(500).json({ success: false, message: "Internal server error" });
  }
};

// Check Farmer Dashboard Status and Restrictions
export const getFarmerDashboardStatus = async (req, res) => {
  try {
    ...
    const userStatus = user.rows[0].status;
    const isApproved = userStatus === "active";
    const hasLocation = farmInfo.rows[0].latitude !== null && farmInfo.rows[0].longitude !== null;
    const hasProofDoc = documents.rows.length > 0 && documents.rows[0].farmer_proof_doc_url !== null;

    // Determine dashboard restrictions and messages
    let dashboardStatus = "active";
    let messageBar = null;
    let restrictions = {
    ...
    if (!hasLocation) {
      dashboardStatus = "incomplete_location";
      messageBar = {
        type: "warning",
        message: "👉 Please update your location to continue.",
        action: "update_location"
      };
    } else if (!hasProofDoc) {
      dashboardStatus = "incomplete_profile";
      messageBar = {
        type: "warning", 
        message: "👉 Please complete your profile by uploading your proof document.",
      ...
      messageBar = {
        type: "info",
        message: "👉 Your account is pending admin approval. You'll have full access once approved.",
        action: null
      };
    } else {
      // Fully approved farmer
      restrictions = {
        canAddProducts: true,
    ...
    res.status(200).json({
      ...
      profile: {
        ...
        coordinates: {
          latitude: farmInfo.rows[0].latitude,
          longitude: farmInfo.rows[0].longitude
        }
      }
    });
  } catch (error) {
    console.error("Get Farmer Dashboard Status Error:", error);
    res.status(500).json({ message: "Internal server error" });
  }
};
...
export const loginFarmer = async (req, res) => {
  try {
    ...
    // Check if verified (email verification still required)
    if (!user.is_verified) {
      return res.status(403).json({ message: "Please verify your email before login" });
    }

    // Allow login even if not approved, but with restrictions
    ...
    res.status(200).json({
      message: isApproved ? "Login successful" : "Login successful - Account pending approval",
      user: {
        id: user.id,
        name: user.name,
        email: user.email,
...
export const getFarmerProfile = async (req, res) => {
  try {
    ...
    const farmerProfile = {
      ...
      isApproved: user.rows[0].status === "active",
      farmName: farmInfo.rows[0].farm_name,
      address: farmInfo.rows[0].address,
      ...
      hasSubmittedDocuments: documents.rows.length > 0 && documents.rows[0].farmer_proof_doc_url !== null,
  ...
  try {
    ...
    if (user.rows[0].status !== "active") {
      return res.status(403).json({ 
        message: "Your account is not approved yet. You cannot add products until admin approval." 
      });
    }
...
export const getFarmerProducts = async (req, res) => {
  try {
    ...
    // Check if farmer is approved - allow viewing products even if not approved
    // but show appropriate message
    const isApproved = user.rows[0].status === "active";
    ...
    if (products.rows.length === 0) {
      return res.status(200).json({
        success: true,
        message: isApproved ? "No products added yet." : "No products added yet. Complete your profile to start adding products.",
        products: [],
        isApproved: isApproved
      });
    }
...
export const updateProducts = async (req, res) => {
  try {
    ...
    if (user.rows[0].status !== "active") {
      return res.status(403).json({ 
        message: "Your account is not approved yet. You cannot update products until admin approval." 
      });
    }
